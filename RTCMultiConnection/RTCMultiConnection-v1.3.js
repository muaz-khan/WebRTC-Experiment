 ...
 (function () {
     window.RTCMultiConnection = function (channel) {
         ...
         function init() {
             ...
             self.config = {
                 ...
                 onmessage: function (e) {
                     ...
                     if (e.data.type === 'text') textReceiver.receive({
                         data: e.data,
                         connection: self
                     });

                     else if (e.data.size || e.data.type === 'file') fileReceiver.receive({
                         data: e.data,
                         connection: self
                     });
         ...
         function joinSession(session) {
             ...
             var extra = self.extra || session.extra || {};
         ...
         function captureUserMedia(callback, _session) {
            ...
            var mediaConfig = {
                 ...
                 onsuccess: function (stream) {
                     ...
                     if (callback) callback(stream, streamid);
         ...
         this.addStream = function (session, socket) {
             captureUserMedia(function (stream, streamid) {
     ...
     function RTCMultiSession(root) {
         ...
         var channels = '--',
             isbroadcaster, isAcceptNewSession = true,
             RTCDataChannels = [];
         ...
         function newPrivateSocket(_config) {
             ...
             var peerConfig = {
                 ...
                 onstream: function (stream) {
                     ...
                     else afterRemoteStreamStartedFlowing();
                 },
             ...
             function onSessionOpened() {
                 ...
                 if (_config.channel) root.channels[_config.userid] = {
                     channel: _config.channel,
                     send: function (data) {
                 ...
                 if (!session.oneway && !session.broadcast && isbroadcaster && channels.split('--').length > 3) defaultSocket.send({
             ...
             function socketResponse(response) {
                 ...
                 if (response.mute || response.unmute) {
                     log(response);
                     document.title = JSON.stringify(response.mute || response.unmute);
                 }
                 ...
                 if (response.playRoleOfBroadcaster) setTimeout(function () {
                     ...
                     sockets = sockets.swapNotTrue();
                     root.dontAttachStream = false;
                 }, 600);

                 if (response.suggestRenegotiation) {
                     var renegotiate = response.renegotiate;
                     if (isData(renegotiate)) createOffer();
                     else root.captureUserMedia(function (stream, streamid) {
                         peer.connection.addStream(stream);
                         createOffer(streamid);
                     }, renegotiate);

                     function createOffer(streamid) {
             ...
             if (channel) {
                 ...
                 if (socket) {
                     ...
                     delete socketObjects[channel];
			          ...
			          // when someone leaves, sometimes sockets[whoLeftId] is set to true
			          // and when is called sockets[whoLeftId].func the app fails
             sockets = sockets.swapNotTrue();
         }
     ...
     function FileReceiver() {
         ...
         this.receive = function (e) {
             ...
             if (!moz) {
                 ...
                 if (connection.onFileProgress) connection.onFileProgress({
                     remaining: packets--,
                     length: numberOfPackets,
     ...
     var RTCPeerConnection = function (options) {
             ...
             peer.onicecandidate = function (event) {
                 if (event && event.candidate && !options.renegotiate) options.onICE(event.candidate);
             };
             ...
             function setConstraints() {
                 ...
                 if (moz) constraints.mandatory[options.onmessage ? 'OfferToReceiveVideo' : 'MozDontOfferDataChannel'] = true;
             }
             setConstraints();
             ...
             function _openOffererChannel() {
                 ...
                 options.channel || 'RTCDataChannel', moz ? {} : {
                     reliable: false
                 });
             ...
             return {
                 ...
                 addICE: function (candidate) {
                     try {
                         ...
                         log('addIceCandidate', '\t\t', candidate, '\t\t', e);
                     }
                 },
     ...
     function getUserMedia(options) {
         ...
         function streaming(stream) {
             ...
             if (currentUserMediaRequest.queueRequests.length) getUserMedia(currentUserMediaRequest.queueRequests.shift());
         }

         if (currentUserMediaRequest.streams[idInstance]) streaming(currentUserMediaRequest.streams[idInstance]);
         else {
	 ...
	 Array.prototype.swapNotTrue = function () {
         var swapped = [],
             arr = this,
             length = arr.length;
         for (var i = 0; i < length; i++)
         if (arr[i] && arr[i] !== true) swapped[swapped.length] = arr[i];
         return swapped;
     };
         ...
         self._getStream = function (e) {
             return {
                 ...
                 _private: function (session, enabled) {
                     ...
                     if (e.socket) e.socket.send({
                         userid: this.userid,
                         mute: !! enabled,
                         unmute: !enabled
                     });
                     else log('No socket to send mute/unmute notification message.');
