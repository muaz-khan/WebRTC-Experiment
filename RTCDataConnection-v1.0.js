...
function RTCDataConnection(configuration) {
    var config = {
        ...
        onChannelMessage: function (data) {
            ...
            if (data.size || data.type === 'file') fileReceiver.receive(data, config);
            else if (configuration.onmessage) configuration.onmessage(data);
        },
        ...
        send: function (data) {
            if (data.size) FileSender.send({
                    ...
                    onFileSent: function (file) {
                        ...
                        if (configuration.onFileSent) configuration.onFileSent(file);
                    }
                });
            else ConnectorUI.send(data);
        }
    };
}
    ...
    return {
        ...
        send: function (message) {
            var length = RTCDataChannels.length,
                data = message.file || JSON.stringify(message);

            if (!length) return;
            for (var i = 0; i < length; i++) RTCDataChannels[i].send(data);
        }
    };
}
