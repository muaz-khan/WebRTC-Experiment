describe('RecordRTC', function() {
    it('audio-recording', function() {
        function addLine() {
            console.log('\x1b[31m%s\x1b[0m ', (new Date).toUTCString() + ':');
        }

        console.log('------------------------------');
        console.log('\x1b[31m%s\x1b[0m ', 'audio-recording');

        browser.driver.get('https://www.webrtc-experiment.com/RecordRTC/tests/audio-recording.html').then(function() {
            ...
            function onRecordingStopped() {
                ...
                browser.wait(function() {
                    ...
                    if (blobSize === 0 || blobSize === '0 Bytes' || blobSize === '0' || !blobSize || blobSize === NaN || blobSize === 'NaN' || !blobSize.toString().length) {
                        addLine();
                        console.log('\x1b[31m%s\x1b[0m ', 'audio-recording failed: ' + blobSize);

                        throw new Error('audio-recording failed: ' + blobSize);
                    }

                    return true;
                }, 1000, 'audio-recording failed.');
            }
        });
    });
});
