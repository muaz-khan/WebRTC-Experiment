describe('RecordRTC', function() {
    it('video-recording', function() {
        function addLine() {
            console.log('\x1b[31m%s\x1b[0m ', (new Date).toUTCString() + ':');
        }

        console.log('------------------------------');
        console.log('\x1b[31m%s\x1b[0m ', 'video-recording');

        browser.driver.get('https://www.webrtc-experiment.com/RecordRTC/tests/video-recording.html').then(function() {
            ...
            function onRecordingStopped() {
                ...
                browser.wait(function() {
                    ...
                    if (blobSize === 0 || blobSize === '0 Bytes' || blobSize === '0' || !blobSize || blobSize === NaN || blobSize === 'NaN' || !blobSize.toString().length) {
                        addLine();
                        console.log('\x1b[31m%s\x1b[0m ', 'video-recording failed: ' + blobSize);

                        throw new Error('video-recording failed: ' + blobSize);
                    }

                    return true;
                }, 1000, 'video-recording failed.');
            }
        });
    });
});
