...
function MediaStreamer(socket, outputVideo) {
    window.requestAnimationFrame = window.webkitRequestAnimationFrame || window.mozRequestAnimationFrame;
    window.MediaSource = window.MediaSource || window.WebKitMediaSource;

    if (!window.MediaSource) throw 'Chrome >=M28 (or Firefox with flag "media.mediasource.enabled=true") is mandatory to test MediaSource API!.';

    // MediaStreamer.stream(file)
    this.stream = function(file) {
        if (!file) throw 'You must link webm media file.';
        if (file.type != 'video/webm') throw 'You can stream only webm videos.';
        File.Send({
            file: file,
            channel: socket,
            interval: 10,
    ...
    var fileReceiver = new File.Receiver({
        onProgress: function(chunk) {
            ...
            FileConverter.DataURLToBlob(chunk.value, function(blob) {
                var reader = new FileReader();
                reader.onload = function(e) {
                    var uint8Array = new Uint8Array(e.target.result);
                    bufferSource.appendBuffer(uint8Array);
                };
    ...
    // MediaStreamer.onstream(chunk/data)
    this.onstream = function(chunk) {
        fileReceiver.receive(chunk);
    };

    var prefix = !!navigator.webkitGetUserMedia ? '' : 'moz', bufferSource;

    var mediaSource = new MediaSource();
    outputVideo.src = window.URL.createObjectURL(mediaSource);

    mediaSource.addEventListener(prefix + 'sourceopen', function() {
        bufferSource = mediaSource.addSourceBuffer('video/webm; codecs="vorbis,vp8"');
