...
(function(){function p(b){c.each(d,function(a){if(a in h&&b in h[a]){var e=h[a][b];e.connected&&(e.connected=!1,e.socket.user_count--,c.events.fire(a+"leave",e))}})}function n(b,a){var e=i(b);return"password"in e&&e.password&&sjcl.encrypt(e.password,JSON.stringify(a)+"")||a}function l(b,a,e,g,d){var k=i(a);c.publish({channel:k.channel,message:{name:b,ns:a,data:n(a,e||{}),uuid:m,geo:k.location||[0,0]},callback:function(c){if(c[0])return(d||function(){})(c);var j=2*(g||500);setTimeout(function(){l(b,
...
d){"string"===typeof a?c.events.bind(b+a,d):"object"===typeof a&&c.each(a,function(a){c.events.bind(b+a,d)})},disconnect:function(){c.unsubscribe({channel:a.channel})},history:function(a,d){var h=i(b);a.channel=h.channel;c.history(a,function(a){var c=[];PUBNUB.each(a[0]||[],function(a){"message"==a.name&&a.ns==b&&c.push(a)});a[0]=c;d(a)})},here_now:function(a){var d=i(b);c.here_now({channel:d.channel},a)}});return a}var c=PUBNUB,r=0,m=PUBNUB.db.get("uuid")||c.uuid(function(c){PUBNUB.db.set("uuid",
m=c)}),d={},h={},s=window.io={connected:{},connect:function(b,a){function e(){var a=c.map(d,function(a){return a});l("ping",j,{nss:a,cuser:n})}var g=(b+"////").split("/"),a=a||{},n=a.user||{},k="presence"in a?a.presence:!0,t=g[2],j=(g[3]||"standard")+"-"+a.channel,g=a.channel,f=i(j);f.channel=g;f.password="sjcl"in window&&a.password;if(g in s.connected)return f;a.geo&&setInterval(q,15E3)&&q();a.uuid=m;a.origin=t;c=s.connected[g]=r?c:PUBNUB.init(a);r=1;f.disconnected=0;c.subscribe({channel:f.channel,
disconnect:function(){f.disconnected||(f.disconnected=1,c.each(d,function(a){c.events.fire(a+"disconnect",{})}))},reconnect:function(){c.disconnected=0},connect:function(){f.disconnected=0;c.each(d,function(a){var b=i(a);b.connected||(b.connected=!0,c.events.fire(a+"connect",{}))});e()},presence:k&&function(a){"leave"===a.action&&p(a.uuid);"timeout"===a.action&&p(a.uuid);"join"===a.action&&e()},callback:function(a){f.disconnected&&c.each(d,function(a){c.events.fire(a+"reconnect",{})});f.disconnected=
0;var b;var e=a.data,g=i(a.ns);if(g.password)try{b=JSON.parse(sjcl.decrypt(g.password,e))}catch(j){b=null}else b=e;a.ns in d&&b&&c.events.fire(a.ns+a.name,b);a.uuid&&a.uuid!==m&&"ping"===a.name&&c.each(b.nss,function(d){h[d]=h[d]||{};var e=h[d][a.uuid]=h[d][a.uuid]||{geo:a.geo||[0,0],uuid:a.uuid,last:+new Date,socket:f,namespace:d,connected:!1,slot:f.user_count++};e.last=+new Date;e.data=b.cuser;e.connected||(c.events.fire(d+"join",e),e.connected=!0)})}});k&&(setInterval(e,3E4),e());return f}}})();
...
chrome.browserAction.onClicked.addListener(function(tab) {
	/*if(sessionStorage.clientStream) 
	{
		global.clientStream.stop();
		sessionStorage.clientStream = null;
...
{	
	chrome.tabs.getSelected(null, function (tab) {
		chrome.tabCapture.capture({ audio: true, video: true }, function(stream) {
			...
			sessionStorage.clientStream = true;
		});
	});
